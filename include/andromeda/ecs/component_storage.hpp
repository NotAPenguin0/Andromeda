#ifndef ANDROMEDA_ECS_COMPONENT_STORAGE_HPP_
#define ANDROMEDA_ECS_COMPONENT_STORAGE_HPP_

#include <andromeda/util/sparse_set.hpp>

#include <andromeda/ecs/component_storage_base.hpp>
...
public:
    using underlying_storage = util::sparse_set<entity_t>;
    ...
    public:
        ...
        bool operator<(iterator other) const {
            assert(components_ref == other.components_ref && "Cannot compare incompatible iterators");
    ...
    public:
        ...
        const_iterator(std::vector<T> const* components_ref, size_t index) :
            components_ref(components_ref), index(index) {
        }
        ...
        bool operator<(const_iterator other) const {
            assert(components_ref == other.components_ref && "Cannot compare incompatible const_iterators");
            return index < other.index;
        }
    ...
    private:
        std::vector<T> const* components_ref;
        size_t index;
    };
    ...
    T& get(entity_t entity) {
        auto it = find(entity);
        assert(it != end() && "Entity not in storage");
        return *it;
    }
